function [A_hat E_hat] = winRPCA_median(I, opt, mu, tol, max_iter)
...
% lambda = 1 / sqrt(max(m1, n1));
%% 
% [A1, E1] = APG_IR(D, lambda);

[A1, E1] = RPCA(D, lambda, mu, tol, max_iter);
...
for i=1:m
    for j=1:n
        if C(i,j) > 0
            %%
%             A_hat(i,j) = mean(AA(i,j,1:C(i,j)));
%             E_hat(i,j) = mean(EE(i,j,1:C(i,j)));
